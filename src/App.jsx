...
const createWavBuffer = (channelData, sampleRate) => {
...
function App() {
  ...
  useEffect(() => {
    if (isPlaying && leftOscRef.current && rightOscRef.current && gainNodeRef.current) {
  ...
  const downloadTone = async () => {
    ...
    try {
      ...
      const offlineContext = new OfflineContextClass(2, frameCount, sampleRate);
        ...
        <section className="grid gap-6 md:grid-cols-2">
          <div className="flex flex-col gap-5">
            <label className="flex flex-col gap-2">
              ...
              <InputNumber
                value={baseFrequency}
                onValueChange={(event) => setBaseFrequency(clamp(event.value ?? 220, 20, 1200))}
                mode="decimal"
                min={20}
                max={1200}
                showButtons
            ...
            <label className="flex flex-col gap-2">
              ...
              <InputNumber
                value={beatFrequency}
                onValueChange={(event) => setBeatFrequency(clamp(event.value ?? 8, 1, 40))}
                mode="decimal"
                min={1}
                max={40}
                showButtons
            ...
            <label className="flex flex-col gap-2">
              ...
              <Slider value={volume} onChange={(event) => setVolume(clamp(event.value, 0, 100))} />
            ...
            <label className="mt-2 flex flex-col gap-2">
              ...
              <InputNumber
                value={duration}
                onValueChange={(event) => setDuration(clamp(event.value ?? 30, 5, 300))}
                min={5}
                max={300}
                showButtons
                buttonLayout="horizontal"
